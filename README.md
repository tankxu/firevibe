...
亚马逊自家的 BLE 遥控器不止一款。Fire TV 固件（`BluetoothKeyMapLib`）里的官方
名单有 11 个 PID，VID 统一是 `0x0171`：

| PID | 按键数 | |
|---|---|---|
| `0x0413` | 16 | 精简款 |
| `0x041E` `0x0421` `0x042F` `0x0431` | 21 | 标准 Alexa 语音遥控 |
| `0x0423` `0x0424` | 39 | 带数字键的电视遥控 |
| `0x0414` `0x0415` `0x0418` | 42 | 同上 |
| `0x0425` | 45 | 功能最全的一款 |

11 款全部带语音键，HID 报告描述符**逐字节相同**，按键完全通用。

不在这份名单里的遥控器，Fire TV 只是不给它下发动态按键映射（影响 App 快捷键、
IR 学习那类定制行为）；方向键、音量、语音这些走标准 BLE HID，照样能用。
对 FireVibe 而言更是无所谓 —— 它只认 HID，配对上就能测绘。

### 两种开麦方式

麦克风的行为却分两派，而且是反的：

| | 热麦克风（如 `0x0421`） | 按住才出声（如 `0x0425`） |
|---|---|---|
| 怎么开 | 主机发一条命令，之后一直出声 | 遥控器自己推，**只在按住麦克风键时** |
| 物理按键 | 无所谓，按不按都出声 | 必须按住，松手立停 |

**应用会自己认出来**：连上后静悄悄试一次，出声就是热麦克风，不出就是按住型，
结果记下来，换遥控器时重新判。

认成「按住才出声」时，麦克风键的语音动作要配在**长按**里 —— 短按点一下就松手，
