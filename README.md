...
**按麦克风键弹出了 Spotlight？**
这个键的 HID 用途码是「搜索」，macOS 会自行响应。FireVibe 会在事件层拦下它，
并且只在**确认这次按键来自遥控器**时才拦 —— 键盘上的搜索键不受影响。

**遥控器时连时断？**
换成 1.5V 碱性电池。1.2V 充电电池无法满足蓝牙射频的峰值电流。

**权限已经开启，却提示没有权限？**
如果是自行从源码构建、并使用了临时（ad-hoc）签名，每次重新构建都会导致授权失效，
而系统设置里的开关看上去仍是开启状态。设置页里的「重置授权」按钮可以修复。
从 Releases 下载的安装包不存在这个问题。

**升级后按键没有反应？**
同上，重新勾选一次「输入监控」。已有的按键配置不会丢失。
