...
//!   - **最多 2 段**（`for i in 0..<2`）
//!   - 时长是**有符号 int16**，但单位是 **10 µs 一格**（见 [`DEVICE_TICK_US`]）
//!     → 单个条目上限 **327670 µs**
//!
//! ❗ 两条早先写在这儿的断言**都是错的**，别照抄：
//!   · 「空调塞不进去」—— 错。「2 段」限制的是 Pronto 那种 intro/repeat，
//!     **和帧数无关**：raw 数组本来就是交替的 mark/space，多帧编成**一段连续序列**
//!     即可，帧间隔就是个长 space。
//!   · 「上限 32767 µs」—— 错，漏了 10 µs 的单位换算，害得码全长了 10 倍。
//!     真实上限 327670 µs，Daikin 那种 25~35 ms 的帧间隔宽裕得很。
//!     （按错的上限筛掉的三千多条码后来全收了回来。）
//!
//! 单位是拿 StackChan 抓码 diff 出来的 —— **设备对错误单位照样 ACK 0x02**，
//! 光看回执判断不了对错。
//!
//! 载荷大小另有上限，而且**两种遥控器不一样**：仿品那条路（写键位表）单条码
//! 约 260 个脉冲封顶，见 [`crate::irtable::MAX_PAYLOAD_BYTES`]；
//! 原厂走 blast，319 脉冲的空调整帧实测通过。
